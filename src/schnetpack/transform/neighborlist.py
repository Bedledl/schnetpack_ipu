...
class KNNNeighborList(TorchNeighborList):
    ...
    def _build_neighbor_list(self, Z, positions, cell, pbc, cutoff):
        ...
        # Sort along first dimension (necessary for atom-wise pooling)
        sorted_idx = torch.argsort(idx_i)
        idx_i = idx_i[sorted_idx]
        idx_j = idx_j[sorted_idx]
    ...
    def _get_neighbor_pairs(self, positions, cell, shifts, cutoff):
        ...
        num_all_atoms = pi_all.shape[0]
        distance_matrix = torch.full((num_atoms, num_atoms), max_distance + 1)

        distance_matrix[pi_all, pj_all] = distances
        distance_matrix[pj_all, pi_all] = distances

        distances, neighbor_pairs = torch.topk(-distance_matrix, self.k)
        distances = - distances

        # TODO shifts!
        #offsets = #shifts_all[pair_index]
        offsets = torch.tensor([[0, 0, 0]]).repeat(num_atoms * self.k, 1)

        idx_i = all_atoms.repeat_interleave(self.k)
        idx_j = neighbor_pairs.flatten()
